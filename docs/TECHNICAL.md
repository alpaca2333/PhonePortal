...
- **外部贴图要「让 loader 自己加载」，不能在解析后自己给 Texture 赋值**（FBX→GLB 子应用）：FBX 把贴图写成外部文件时，只丢 `.fbx` 进来会让 `FBXLoader` 去问页面要一张不存在的图（404）→ 材质槽空着。正确做法是**在 `LoadingManager` 的 URL 重写钩子里**把引用换成用户提供文件的 `blob:` URL：`FBXLoader` 会把外部引用化简成纯文件名，而 three 的 `ImageLoader` 在加载前一定调用 `manager.resolveURL(url)`，于是贴图是 **three 自己按它自己的代码加载的**——sRGB 颜色空间（`DiffuseColor`）、FBX 的 wrap 模式、`flipY`、`needsUpdate` 全部与「内嵌贴图」走同一条路。**为什么这点重要**：事后自己给 `Texture` 赋 `image` 就得自己复刻这套标记，两份实现迟早漂移（颜色变暗/贴图上下颠倒这类问题不会报错，只会画错）。**但重写不是万能的**：`FBXLoader` 对 `.tga`/`.psd`/`.dds` 会**静默造占位 Texture 且根本不问 manager**（`getHandler('.tga') === null` 分支），所以要有第二条兜底路线——解析后按 Texture 自己的名字再匹配一次（`body_diffuse.tga` 引用 + `body_diffuse.png` 文件）。**顺带一个只有写测试才会知道的三方细节**：three 的 `ImageLoader` 在自己的加载回调里用了 `this`（`onLoad(this)`），所以任何伪造 `<img>` 的测试替身必须像真实 DOM 那样 `f.call(element, event)` 派发事件，否则 `texture.image` 被赋成 `undefined` 且**不报任何错**；同理 `GLTFLoader` 处理内嵌图片时会读全局 `self`（`self.URL`），Node 里没有 `self`。
- **「先 parse 再装 onLoad」会错过已经开始的加载**（同一个子应用，被测试抓到的第二个坑）：贴图请求是在 `loader.parse()` **内部同步发起**的，所以「parse → 再设 `manager.onLoad` → 等它」会漏掉完成事件，症状是报告说「已补 1 张贴图」而材质槽仍为空（图像晚一个 tick 到达）。修法是把这件事收成一个 **session**：`onLoad` 在 parse **之前**装好，`done()` 只负责等，并用「有没有请求过」区分「这个文件没有贴图」与「还在加载」。**通用规则**：任何「先动作、后挂回调」的等待都要问一句「回调会不会在我挂上之前就触发」。
- **手写测试夹具时，FBX 的连接方向是「子在前、父在后」**（`C: "OO",VideoID,TextureID`）：反向写（`TextureID,VideoID`）不会报错，只是 `connections.get(texture).children` 变成空数组、外部贴图引用解析成 `undefined` + `''` 拼成字符串 `"undefined"` 去加载——**一个静默的哑失败，表现为「贴图名是 undefined」**。夹具里的每条 `C:` 都要问一句方向的语义（`OO` = 子→父，`OP` = 子→父 + 属性名）。
- **要给蒙皮模型减面，别用 three 自带的 `SimplifyModifier`**（FBX→GLB 子应用）：它开头就是
  `for (const name in attributes) if (name !== 'position' && name !== 'uv' && name !== 'normal' …) geometry.deleteAttribute(name)`
  —— 也就是说 **`skinIndex` / `skinWeight` 会被直接删掉**，蒙皮角色一减面就变成"不会动的静态网格"（不报错，
  只是从此不跟骨骼动），而且它不认 UV 缝合线、还丢掉多材质分组。**用 meshoptimizer 的 simplifier**（vendored，
  wasm 以压缩字符串内嵌在同一个 `.js` 里，不发网络请求）：它**只重写索引缓冲**，顶点数据原样保留，所以
  `skinIndex`/`skinWeight`/`uv` 天然仍然有效；再用 `simplifyWithAttributes` 把 uv（权重 1）与蒙皮权重
  （权重 0.01）当"属性"交给它，它就会避免跨缝合线/权重突变处折叠（gltfpack 用的就是这条路）。**两条必须
  记住的推论**：① `target_error`（误差上限）先到就先停，所以"保留 20%"是目标不是保证（实测 1% 误差下只减到
...
- **要"压缩贴图"，别用 three 导出器的 `maxTextureSize`，而且体积的真正开关是贴图不是面数**（FBX→GLB 子应用）：
  `GLTFExporter` 的 `processImage` 是 `canvas.width = Math.min(image.width, options.maxTextureSize)`、
  高度同理 —— 宽高**各自**截断，8192×2048 的贴图会被拉成 2048×2048（比例变了、图被压扁）。所以降分辨率要
  自己按**最长边**等比缩放到 canvas（`drawImage` 一次画完），再把 canvas 当 `texture.image` 交出去。
  编码格式则由导出器读的 `texture.userData.mimeType` 决定（不透明贴图给 `image/jpeg`，带 alpha 的槽、
  法线贴图、`transparent` 材质保持 PNG）——这是**唯一**能在导出时换编码的钩子，`image/webp` 会被它降级回
  PNG。**三个容易忽略的点**：① **共享图片必须只压一次**（同一张 image 被 `map`/`emissiveMap` 共用时，
  各压一遍会让导出器按 image 对象去重失效 → GLB 里嵌两份、体积翻倍），按"源 image 对象"缓存即可；
  ② **内存护栏**：把 8192×8192 原样搬进 canvas 再编码要 ~270MB（手机直接 OOM），所以 canvas 像素数要有
  上限并**跳过 + 说明**，而不是把页面拖死；③ **画布污染**：跨域图片会让 `toBlob` 抛 SecurityError，而那是
  在**导出器内部**炸 → 整个导出失败，所以在赋值前 `getImageData(0,0,1,1)` 探一下，把全局失败降级成跳过一张。
  **顺带记一条体量上的经验**：用户那个 264MB 的角色，几何数据只有几 MB（46k 面 + 139k 顶点），**99% 是贴图**
  —— 「减面」降的是 GPU 负载，「压贴图」才是降文件体积的那一刀，两者不能互相代替。
